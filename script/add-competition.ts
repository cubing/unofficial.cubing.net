...
const html = sharedDOMParser.parseFromString( await (await fetch(competitionWCAURL)).text(), "text/html");

const fullName = html.querySelector("#competition-data h3")?.textContent?.trim();
if (!fullName) {
  console.error("Could not look up the competition name. Please ensure the competition exists on the WCA website: " + competitionWCAURL);
  exit(1)
}
console.log("Found competition at: "+ competitionWCAURL)
const date = html.querySelector("a[title=\"Add to calendar\"]")!.parentElement!.textContent!.trim() + " // change this to the format `YYYY-MM-DD`! For a multi-day comp, pick the final day of the competition if unsure on which date the specific event ended.";

let roundsByEvent: Record<EventID, CompetitionRoundInfo[]> = {}
...
const outputFilePath = (await COMPETITON_SOURCE_DATA_FOLDER.getRelative(competitionID).ensureFolderExists()).getRelative("competition-info.json");
